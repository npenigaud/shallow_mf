...
SELECT CASE(KVERB)
  ...
  CASE(NVERB_DEBUG)
    YPRE='DEBUG:   '
  CASE DEFAULT
    WRITE(UNIT=*, FMT=*) 'ERROR: PRINT_MSG: wrong verbosity level'
END SELECT
!
YSUBR=TRIM(HSUBR)//':'

IF (ILINES==1) THEN
  WRITE(UNIT=*, FMT="(A9,A30,A)") YPRE, YSUBR, TRIM(HMSG(1))
ELSE
 IF (ILINES<10) THEN
    YSZ = 'I1'
  ...
  DO JI=1, ILINES
    WRITE(UNIT=*, FMT=YFORMAT) YPRE, YSUBR, JI, ILINES, TRIM(HMSG(JI))
  ENDDO
ENDIF
!
IF (KVERB<=N_ABORT_LEVEL) THEN
  PRINT *, TRIM(HMSG(ILINES))
  STOP 1
END IF
!
