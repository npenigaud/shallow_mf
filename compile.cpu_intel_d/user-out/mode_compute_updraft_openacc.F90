...
&, ONOMIXLG, KSV_LGBEG, KSV_LGEND, PZZ, PDZZ, PSFTH, PSFRV, PEXNM, PPABSM, PRHODREF, PUM&
&, PVM, PTKEM, PTHM, PRVM, PTHLM, PRTM, PSVM, PTH_UP, PTHL_UP, PRT_UP, PRV_UP, PRC_UP, PRI_UP&
&, PTHV_UP, PW_UP, PU_UP, PV_UP, PSV_UP, PFRAC_UP, PFRAC_ICE_UP, PRSAT_UP, PTKE_UP, PEMF&
&, PDETR, PENTR, PBUO_INTEG, KKLCL, KKETL, KKCTL, PDEPTH, PDX, PDY, YDSTACK)
...
USE MODE_SHUMAN_MF_OPENACC,ONLY:MZM_MF_OPENACC, MZF_MF_OPENACC, GZ_M_W_MF_OPENACC
USE MODE_COMPUTE_BL89_ML_OPENACC,ONLY:COMPUTE_BL89_ML_OPENACC
USE MODE_MSG,ONLY:PRINT_MSG, NVERB_FATAL

USE BR_INTRINSICS, ONLY : BR_EXP, BR_LOG, BR_POW, BR_TANH
#include "stack.h"
USE STACK_MOD
USE ABOR1_ACC_MOD
...
REAL, INTENT (OUT)::PTH_UP(D%NIJT, D%NKT)
REAL, INTENT (OUT)::PV_UP(D%NIJT, D%NKT)
REAL, INTENT (OUT)::PU_UP(D%NIJT, D%NKT)
REAL, INTENT (OUT)::PTKE_UP(D%NIJT, D%NKT)
...
temp (REAL, ZEXN_F, (D%NIJT, D%NKT))
temp (REAL, ZG_O_THVREF_UP, (D%NIJT, D%NKT))
...
REAL::ZMIX2
REAL::ZMIX1
temp (REAL, ZLUPSURF, (D%NIJT))
temp (REAL, ZLDOWN, (D%NIJT))
...
REAL::ZT
REAL::ZWK0D
REAL::ZCOEFF_PLUS_HALF_THL
REAL::ZCOEFF_MINUS_HALF_THL
REAL::ZCOEFF_PLUS_HALF_RT
REAL::ZCOEFF_MINUS_HALF_RT
REAL::ZCOEFF_PLUS_HALF_THV
REAL::ZCOEFF_MINUS_HALF_THV
REAL::ZPRE
REAL::ZG_O_THVREF_ED
REAL::ZFRAC_ICE
REAL::ZTHV_PLUS_HALF
...
IF (KIND (ZEXN_F) == 8) THEN
    alloc8 (ZEXN_F)
ELSEIF (KIND (ZEXN_F) == 4) THEN
    alloc4 (ZEXN_F)
ELSE
    STOP 1
ENDIF
...
IF (KIND (ZG_O_THVREF_UP) == 8) THEN
    alloc8 (ZG_O_THVREF_UP)
ELSEIF (KIND (ZG_O_THVREF_UP) == 4) THEN
    alloc4 (ZG_O_THVREF_UP)
ELSE
    STOP 1
ENDIF
...
IF (KIND (ZLUPSURF) == 8) THEN
    alloc8 (ZLUPSURF)
ELSEIF (KIND (ZLUPSURF) == 4) THEN
    alloc4 (ZLUPSURF)
ELSE
    STOP 1
ENDIF
...
IF (KIND (ZLDOWN) == 8) THEN
    alloc8 (ZLDOWN)
ELSEIF (KIND (ZLDOWN) == 4) THEN
    alloc4 (ZLDOWN)
ELSE
    STOP 1
ENDIF
...
IF (KIND (KDEPTH) == 8) THEN
    alloc8 (KDEPTH)
ELSEIF (KIND (KDEPTH) == 4) THEN
    alloc4 (KDEPTH)
ELSE
    STOP 1
ENDIF
...
IF (OENTR_DETR) THEN
  ...
  CALL MZM_MF_OPENACC (D, PEXNM, ZEXN_F, YDSTACK=YLSTACK)

  IF (PARAMMF%LPZ_EXP_LOG) THEN

    DO JK=1, IKT
      
      ZWK (JI, JK)=BR_LOG (PPABSM (JI, JK))
    
    ENDDO

    CALL MZM_MF_OPENACC (D, ZWK, ZPRES_F, YDSTACK=YLSTACK)

    DO JK=1, IKT
      
      ZPRES_F (JI, JK)=BR_EXP (ZPRES_F (JI, JK))
  ...
  ZBUF (IEXN)=BR_POW((ZPRES_F (JI, IKB)/CST%XP00), CST%RDSCPD)
  ...
  PTH_UP (JI, IKB)=PTHL_UP (JI, IKB)+ZBUF (ILVOCPEXN)*PRC_UP&
  ...
  DO II=1, JITER
    ...
    ZBUF (IT)=PTH_UP (JI, IKB)*ZBUF (IEXN)
    ...
    ZBUF (ILOGT)=BR_LOG (ZBUF (IT))
    
    ZBUF (IFOESW)=MIN (BR_EXP (CST%XALPW-CST%XBETAW/ZBUF (IT)-CST%XGAMW*ZBUF (ILOGT)), ZBUF (I99PP))
    ZBUF (IFOESI)=MIN (BR_EXP (CST%XALPI-CST%XBETAI/ZBUF (IT)-CST%XGAMI*ZBUF (ILOGT)), ZBUF (I99PP))
    ...
    PTH_UP (JI, IKB)=PTHL_UP (JI, IKB)+ZBUF (ILVOCPEXN)*PRC_UP&
    & (JI, IKB)+ZBUF (ILSOCPEXN)*PRI_UP (JI, IKB)
    ZVAR1=PTH_UP (JI, IKB)*ZBUF (IEXN)-ZBUF (IT)
  ...
  PTHV_UP (JI, IKB)=PTH_UP (JI, IKB)*((1+ZRVORD*PRV_UP (JI, IKB))/(1+PRT_UP (JI, IKB)))
  ...
  DO JK=1, IKT
    ...
    ZG_O_THVREF_UP (JI, JK)=CST%XG/PTHV_UP (JI, JK)
  ...
  IF (TURBN%CTURBLEN=='RM17') THEN
    ...
    DO JK=1, IKT
      ...
      ZSHEAR (JI, JK)=SQRT (BR_POW(ZDUDZ (JI, JK), 2)+BR_POW(ZDVDZ (JI, JK), 2))
  ...
  & (:, IKB), ZTHVM, IKB, GLMIX,.TRUE., ZSHEAR, ZLUPSURF, YDSTACK=YLSTACK)
  
  ZLUPSURF (JI)=MAX (ZLUPSURF (JI), 1.E-10)
  ...
  IF (PARAMMF%LGZ) THEN
    ...
    ZSURF =BR_TANH (PARAMMF%XGZ*SQRT (PDX*PDY)/ZLUPSURF (JI))
  ...
  IF (ZWTHVSURF >0.) THEN
    PEMF (JI, IKB)=PARAMMF%XCMF*ZSURF *ZRHO_F (JI, IKB)*BR_POW(((ZG_O_THVREF&
    & (JI, IKB))*ZWTHVSURF *ZLUPSURF (JI)), (1./3.))
    PFRAC_UP (JI, IKB)=MIN (PEMF (JI, IKB)/(SQRT (ZW_UP2 (JI&
    &, IKB))*ZRHO_F (JI, IKB)), PARAMMF%XFRAC_UP_MAX)
    ZW_UP2 (JI, IKB)=BR_POW((PEMF (JI, IKB)/(PFRAC_UP (JI, IKB)*ZRHO_F (JI, IKB))), 2)
    GTEST =.TRUE.
  ELSE
    PEMF (JI, IKB)=0.
    GTEST =.FALSE.
...
DO JK=IKB, IKE-IKL, IKL
  ...
  IF (OENTR_DETR) THEN
    ...
    IF (PARAMMF%CWET_MIXING=='LR01'.OR.PARAMMF%CDETR_DRY_LUP=='UPDR') THEN
      CALL MZF_MF_OPENACC (D, PTHV_UP, ZWK, YDSTACK=YLSTACK)
      CALL COMPUTE_BL89_ML_OPENACC (D, CST, CSTURB, PDZZ, ZTKEM_F (:, JK), ZG_O_THVREF_UP&
      & (:, JK), ZWK, JK,.TRUE.,.TRUE., ZSHEAR, ZLUP, YDSTACK=YLSTACK)
      
      ZLUP (JI)=MAX (ZLUP (JI), 1.)
    
    ENDIF


    IF (PARAMMF%CWET_MIXING=='LR01') THEN
      CALL MZF_MF_OPENACC (D, ZTHVM_F, ZWK, YDSTACK=YLSTACK)
      CALL MZF_MF_OPENACC (D, PTHV_UP, ZWK2, YDSTACK=YLSTACK)
      ZWK (JI, JK)=ZWK2 (JI, JK)
      CALL COMPUTE_BL89_ML_OPENACC (D, CST, CSTURB, PDZZ, ZTKEM_F (:, JK), ZG_O_THVREF_UP&
      & (:, JK), ZWK, JK,.FALSE.,.FALSE., ZSHEAR, ZLDOWN, YDSTACK=YLSTACK)
      
      ZLDOWN (JI)=MAX (ZLDOWN (JI), 1.)
    ...
    ELSEIF (GTEST .AND..NOT.GTESTLCL ) THEN
      ZT=PTH_UP (JI, JK)*BR_POW((ZPRES_F (JI, JK)/CST%XP00), (CST%XRD/CST%XCPD))
      ZFOESW=MIN (BR_EXP (CST%XALPW-CST%XBETAW/ZT-CST%XGAMW*BR_LOG (ZT)), 0.99*ZPRES_F (JI, JK))
      ZFOESI=MIN (BR_EXP (CST%XALPI-CST%XBETAI/ZT-CST%XGAMI*BR_LOG (ZT)), 0.99*ZPRES_F (JI, JK))
      ...
      IF (PARAMMF%LPZ_EXP_LOG) THEN
        ZPART_DRY =MAX (0., MIN (1.,(BR_LOG (ZPRES_F (JI, JK))-BR_LOG (ZPRE&
        & ))/(BR_LOG (ZPRES_F (JI, JK))-BR_LOG (ZPRES_F (JI, JK+IKL)))))
      ELSE
    ...
    IF (JK/=IKB) THEN
      ZCOEFF_MINUS_HALF_THV =((ZTHVM (JI, JK)-ZTHVM (JI, JK-IKL))/PDZZ (JI, JK))
      ZTHV_MINUS_HALF =ZTHVM (JI, JK)-ZCOEFF_MINUS_HALF_THV *0.5*(PZZ (JI, JK+IKL)-PZZ (JI, JK))
    ELSE
      ZCOEFF_MINUS_HALF_THV =0.
      ZTHV_MINUS_HALF =ZTHVM (JI, JK)
    ENDIF

    ZCOEFF_PLUS_HALF_THV =((ZTHVM (JI, JK+IKL)-ZTHVM (JI, JK))/PDZZ (JI, JK+IKL))
    ZTHV_PLUS_HALF =ZTHVM (JI, JK)+ZCOEFF_PLUS_HALF_THV *0.5*(PZZ (JI, JK+IKL)-PZZ (JI, JK))
    ...
    IF (GTEST .AND.ZPART_DRY >0.) THEN
      ...
      ZBUO_INTEG_DRY (JI, JK)=ZG_O_THVREF_ED *ZDZ*(0.5*(-ZCOEFF_MINUS_HALF_THV&
      & )*ZDZ-ZTHV_MINUS_HALF +PTHV_UP (JI, JK))
      ZDZ=MAX (0., ZDZ_STOP -(PZZ (JI, JK+IKL)-PZZ (JI, JK))*0.5)
      ZBUO_INTEG_DRY (JI, JK)=ZBUO_INTEG_DRY (JI, JK)+ZG_O_THVREF_ED *ZDZ*(0&
      &.5*(-ZCOEFF_PLUS_HALF_THV )*ZDZ-ZTHVM (JI, JK)+PTHV_UP (JI, JK))

      IF (ZBUO_INTEG_DRY (JI, JK)>=0.) THEN
        PENTR (JI, JK)=0.5/(PARAMMF%XABUO-PARAMMF%XBENTR*PARAMMF%XENTR_DRY)*BR_LOG (1.+(2.*(PARAMMF&
        &%XABUO-PARAMMF%XBENTR*PARAMMF%XENTR_DRY)/ZW_UP2 (JI, JK))*ZBUO_INTEG_DRY (JI, JK))
        PDETR (JI, JK)=0.
      ELSE
        PENTR (JI, JK)=0.
        PDETR (JI, JK)=0.5/(PARAMMF%XABUO)*BR_LOG (1.+(2.*(PARAMMF&
      ...
      IF (PARAMMF%CDETR_DRY_LUP=='SURF') THEN
        ZWK0D=ZLUPSURF (JI)-0.5*(PZZ (JI, JK)+PZZ (JI, JK+IKL))
    ...
    ZBUF (IEXN)=BR_POW((ZPRES_F (JI, JK+IKL)/CST%XP00), CST%RDSCPD)
    ...
    IF (GTEST .AND.ZPART_DRY <1.) THEN
      ...
      ZBUO_INTEG_CLD (JI, JK)=ZG_O_THVREF_ED *ZDZ*(0.5*(ZCOTHVU-ZCOEFF_MINUS_HALF_THV&
      & )*ZDZ-(ZTHVM (JI, JK)-ZDZ*ZCOEFF_MINUS_HALF_THV )+PTHV_UP (JI, JK))
      ZDZ=(PZZ (JI, JK+IKL)-PZZ (JI, JK))-MAX (ZDZ_STOP , 0.5*(PZZ (JI, JK+IKL)-PZZ (JI, JK)))
      ZBUO_INTEG_CLD (JI, JK)=ZBUO_INTEG_CLD (JI, JK)+ZG_O_THVREF_ED *ZDZ*(0.5*(ZCOTHVU&
      &-ZCOEFF_PLUS_HALF_THV )*ZDZ-(ZTHVM (JI, JK)+(0.5*((PZZ (JI, JK+IKL)-PZZ (JI&
      &, JK)))-ZDZ)*ZCOEFF_PLUS_HALF_THV )+(ZTHV_UP_F2 -ZDZ*ZCOTHVU))
    ELSE
    ...
    IF (PARAMMF%CKIC_COMPUTE=='KFB') THEN
      ...
      IF (GTEST .AND.ZPART_DRY >0.5) THEN
        ...
        ZTHV =ZTHVM (JI, JK)+ZCOEFF_PLUS_HALF_THV *ZDZ
      ...
      ELSEIF (GTEST ) THEN
        ...
        ZTHV =ZTHVM (JI, JK)-ZCOEFF_MINUS_HALF_THV *ZDZ
      ...
      ZBUF (IEXN)=BR_POW((ZPRE /CST%XP00), CST%RDSCPD)
    ...
    ELSEIF (PARAMMF%CKIC_COMPUTE=='RS08') THEN
      ...
      IF (GTEST ) THEN
        ...
        IF (JK/=IKB) THEN
          ZCOEFF_MINUS_HALF_RT =((PRTM (JI, JK)-PRTM (JI, JK-IKL))/PDZZ (JI, JK))
          ZCOEFF_MINUS_HALF_THL =((PTHLM (JI, JK)-PTHLM (JI, JK-IKL))/PDZZ (JI, JK))
        ELSE
          ZCOEFF_MINUS_HALF_RT =0.
          ZCOEFF_MINUS_HALF_THL =0.
        ENDIF

        ZCOEFF_PLUS_HALF_RT =((PRTM (JI, JK+IKL)-PRTM (JI, JK))/PDZZ (JI, JK+IKL))
        ZCOEFF_PLUS_HALF_THL =((PTHLM (JI, JK+IKL)-PTHLM (JI, JK))/PDZZ (JI, JK+IKL))

        IF (ZPART_DRY >0.5) THEN
          ZDZ=ZDZ_STOP -0.5*(PZZ (JI, JK+IKL)-PZZ (JI, JK))
          ZTHV =ZTHVM (JI, JK)+ZCOEFF_PLUS_HALF_THV *ZDZ
          ZTHL=PTHLM (JI, JK)+ZCOEFF_PLUS_HALF_THL *ZDZ
          ZRT=PRTM (JI, JK)+ZCOEFF_PLUS_HALF_RT *ZDZ
        ELSE
          ZDZ=0.5*(PZZ (JI, JK+IKL)-PZZ (JI, JK))-ZDZ_STOP 
          ZTHV =ZTHVM (JI, JK)-ZCOEFF_MINUS_HALF_THV *ZDZ
          ZTHL=PTHLM (JI, JK)-ZCOEFF_MINUS_HALF_THL *ZDZ
          ZRT=PRTM (JI, JK)-ZCOEFF_MINUS_HALF_RT *ZDZ
        ENDIF

        ZT=PTH_UP (JI, JK)*ZEXN_F (JI, JK)
        ZLV=(CST%XLVTT+(CST%XCPV-CST%XCL)*(ZT-CST%XTT))
        ZLS=(CST%XLSTT+(CST%XCPV-CST%XCI)*(ZT-CST%XTT))
        ZLVS=(1.-ZFRAC_ICE )*ZLV+ZFRAC_ICE *ZLS
        ZDRSATDT=ZLVS*PRSAT_UP (JI, JK)/(CST%XRV*BR_POW(ZT, 2))*(1.+(CST%XRV*PRSAT_UP (JI, JK)/CST%XRD))
        ZGAMMA=(ZLVS/CST%XCPD)*ZDRSATDT
        ZALPHA=(CST%XCPD/ZLVS)*ZEXN_F (JI, JK)*PTHL_UP (JI, JK)
        ZLAMBDA=CST%XRV/CST%XRD-1.
        ZBETA=1./(1+ZGAMMA)*(1.+(1+ZLAMBDA)*ZGAMMA*ZALPHA)
        ZKIC =(PTHV_UP (JI, JK)-ZTHV )/(ZBETA*(PTHL_UP (JI, JK)-ZTHL)+(ZBETA&
        &-ZALPHA)*ZLVS/(CST%XCPD*ZEXN_F (JI, JK))*(PRT_UP (JI, JK)-ZRT))
        ZKIC =MAX (MIN (ZKIC , 1.), 0.)
      ENDIF
    ...
    IF (GTEST ) THEN
      ...
      IF (PARAMMF%CWET_MIXING=='LR01') THEN
        ZCOEFFMF_CLOUD=PARAMMF%XENTR_MF/(2.*(ZLUP (JI)*ZLDOWN (JI))/(ZLUP (JI)+ZLDOWN (JI)))
        ZENTR_CLD (JI, JK)=(1.-ZPART_DRY )*ZCOEFFMF_CLOUD*ZEPSI 
        ZDETR_CLD (JI, JK)=(1.-ZPART_DRY )*ZCOEFFMF_CLOUD*ZDELTA 
    ...
    IF (GTEST ) THEN
      ...
      PEMF (JI, JK+IKL)=PEMF (JI, JK)*BR_EXP (2*ZMIX1 )
    ENDIF
  ...
  IF (PARAMMF%LRELAX_ALPHA_MF) THEN
    ...
    ZRELAX_ALPHA_COEFF =1./(1.-PFRAC_UP (JI, JK))
  
  ELSE
    ZRELAX_ALPHA_COEFF =1.
  ENDIF
  ...
  IF (GTEST ) THEN
    ...
    ZMIX3_CLD =(PZZ (JI, JK+IKL)-PZZ (JI, JK))*ZDETR_CLD (JI, JK)
    ZMIX2_CLD =(PZZ (JI, JK+IKL)-PZZ (JI, JK))*ZENTR_CLD (JI, JK)
    PTHL_UP (JI, JK+IKL)=(PTHL_UP (JI, JK)*(1.-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PTHLM&
    & (JI, JK)*ZRELAX_ALPHA_COEFF *ZMIX2 )/(1.+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
    PRT_UP (JI, JK+IKL)=(PRT_UP (JI, JK)*(1.-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PRTM&
    & (JI, JK)*ZRELAX_ALPHA_COEFF *ZMIX2 )/(1.+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
  ENDIF
  ...
  IF (PARAMMF%LMIXUV) THEN
    ...
    IF (JK/=IKB) THEN
      ...
      IF (GTEST ) THEN
        PU_UP (JI, JK+IKL)=(PU_UP (JI, JK)*(1-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PUM (JI, JK)*ZRELAX_ALPHA_COEFF&
        & *ZMIX2 +0.5*PARAMMF%XPRES_UV*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*((PUM (JI, JK+IKL)-PUM (JI, JK))/PDZZ&
        & (JI, JK+IKL)+(PUM (JI, JK)-PUM (JI, JK-IKL))/PDZZ (JI, JK)))/(1+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
        PV_UP (JI, JK+IKL)=(PV_UP (JI, JK)*(1-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PVM (JI, JK)*ZRELAX_ALPHA_COEFF&
        & *ZMIX2 +0.5*PARAMMF%XPRES_UV*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*((PVM (JI, JK+IKL)-PVM (JI, JK))/PDZZ&
        & (JI, JK+IKL)+(PVM (JI, JK)-PVM (JI, JK-IKL))/PDZZ (JI, JK)))/(1+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
      ENDIF
    ...
    ELSE
      ...
      IF (GTEST ) THEN
        PU_UP (JI, JK+IKL)=(PU_UP (JI, JK)*(1-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PUM (JI, JK)*ZRELAX_ALPHA_COEFF&
        & *ZMIX2 +0.5*PARAMMF%XPRES_UV*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*((PUM (JI, JK+IKL)-PUM&
        & (JI, JK))/PDZZ (JI, JK+IKL)))/(1+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
        PV_UP (JI, JK+IKL)=(PV_UP (JI, JK)*(1-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PVM (JI, JK)*ZRELAX_ALPHA_COEFF&
        & *ZMIX2 +0.5*PARAMMF%XPRES_UV*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*((PVM (JI, JK+IKL)-PVM&
        & (JI, JK))/PDZZ (JI, JK+IKL)))/(1+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
      ENDIF
  ...
  IF (PARAMMF%LMIXTKE) THEN
    ...
    IF (GTEST ) THEN
      PTKE_UP (JI, JK+IKL)=(PTKE_UP (JI, JK)*(1-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+ZTKEM_F&
      & (JI, JK)*ZRELAX_ALPHA_COEFF *ZMIX2 )/(1+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
    ENDIF
  ...
  DO JSV=1, KSV
    ...
    IF (GTEST ) THEN
      PSV_UP (JI, JK+IKL, JSV)=(PSV_UP (JI, JK, JSV)*(1-0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )+PSVM&
      & (JI, JK, JSV)*ZRELAX_ALPHA_COEFF *ZMIX2 )/(1+0.5*ZRELAX_ALPHA_COEFF *ZMIX2 )
    ENDIF
  ...
  IF (OENTR_DETR) THEN
    ...
    ZBUF (ILVOCPEXN)=(CST%XLVTT+(CST%XCPV-CST%XCL)*ZDELT)/ZVAR2
    ZBUF (ILSOCPEXN)=(CST%XLSTT+(CST%XCPV-CST%XCI)*ZDELT)/ZVAR2
    PTH_UP (JI, JK+IKL)=PTHL_UP (JI, JK+IKL)+ZBUF (ILVOCPEXN)*ZRC_UP +ZBUF (ILSOCPEXN)*ZRI_UP 
    ...
    DO II=1, JITER
      ...
      ZBUF (IT)=PTH_UP (JI, JK+IKL)*ZBUF (IEXN)
      ...
      ZBUF (ILSOCPEXN)=(CST%XLSTT+(CST%XCPV-CST%XCI)*(ZBUF (IT)-CST%XTT))/ZVAR2
      PTH_UP (JI, JK+IKL)=PTHL_UP (JI, JK+IKL)+ZBUF (ILVOCPEXN)*ZRC_UP +ZBUF (ILSOCPEXN)*ZRI_UP 
      ZVAR1=PTH_UP (JI, JK+IKL)*ZBUF (IEXN)-ZBUF (IT)
      ZRSATW =ZRSATW +ZBUF (IDRSATODTW)*ZVAR1
      ZRSATI =ZRSATI +ZBUF (IDRSATODTI)*ZVAR1
    ...
    IF (GTEST ) THEN
      PTHV_UP (JI, JK+IKL)=PTH_UP (JI, JK+IKL)*((1+ZRVORD*PRV_UP (JI, JK+IKL))/(1+PRT_UP (JI, JK+IKL)))
      ZLVERT=MAX (10., SQRT (PFRAC_UP (JI, JK)*PDX*PDY/CST%XPI))
      ZGAMMA_WUP=1.+PARAMMF%XBRIO*BR_POW(ZRELAX_ALPHA_COEFF , 2)/((1-PARAMMF&
      &%XAADVEC)*ZLVERT)*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*ZPART_DRY 
      ZDELTA_WUP=1.-PARAMMF%XBRIO*BR_POW(ZRELAX_ALPHA_COEFF , 2)/((1-PARAMMF&
      &%XAADVEC)*ZLVERT)*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*ZPART_DRY 

      IF (ZBUO_INTEG_DRY (JI, JK)>0.) THEN
        ZW_UP2 (JI, JK+IKL)=ZW_UP2 (JI, JK)*ZDELTA_WUP/ZGAMMA_WUP+2./((1.-PARAMMF%XAADVEC)*ZGAMMA_WUP)*&
        &(PARAMMF%XABUO-PARAMMF%XBENTR*PARAMMF%XENTR_DRY*ZRELAX_ALPHA_COEFF )*ZBUO_INTEG_DRY (JI, JK)
      ELSE
        ZW_UP2 (JI, JK+IKL)=ZW_UP2 (JI, JK)*ZDELTA_WUP/ZGAMMA_WUP+2./((1.-PARAMMF&
        &%XAADVEC)*ZGAMMA_WUP)*PARAMMF%XABUO*ZBUO_INTEG_DRY (JI, JK)
      ENDIF

      ZGAMMA_WUP=1.+ZRELAX_ALPHA_COEFF /(1.-PARAMMF%XAADVEC)*(PARAMMF%XBRIO/ZLVERT*ZRELAX_ALPHA_COEFF *(PZZ&
      & (JI, JK+IKL)-PZZ (JI, JK))*(1.-ZPART_DRY )+PARAMMF%XBDETR*ZMIX3_CLD +PARAMMF%XBENTR*ZMIX2_CLD )
      ZDELTA_WUP=1.-ZRELAX_ALPHA_COEFF /(1.-PARAMMF%XAADVEC)*(PARAMMF%XBRIO/ZLVERT*ZRELAX_ALPHA_COEFF *(PZZ&
      & (JI, JK+IKL)-PZZ (JI, JK))*(1.-ZPART_DRY )+PARAMMF%XBDETR*ZMIX3_CLD +PARAMMF%XBENTR*ZMIX2_CLD )
      ZW_UP2 (JI, JK+IKL)=ZW_UP2 (JI, JK+IKL)*ZDELTA_WUP/ZGAMMA_WUP+2.*PARAMMF&
      &%XABUO*ZBUO_INTEG_CLD (JI, JK)/((1-PARAMMF%XAADVEC)*ZGAMMA_WUP)
    ENDIF
...
IF (OENTR_DETR) THEN
  ...
  IF (PARAMMF%LVERLIMUP) THEN
    ...
    DO JK=1, IKT
      ...
      KDEPTH (JI, JK)=MIN (MAX (0., PZZ (JI, JK)-PZZ (JI, KKLCL (JI))), PDEPTH (JI))
  ...
  DO JK=1, IKT
    ...
    IF (PARAMMF%LVERLIMUP) THEN
      ZCOEF (JI, JK)=(1.-(KDEPTH (JI, JK)-ZDEPTH_MAX1)/(ZDEPTH_MAX2-ZDEPTH_MAX1))
    ELSE
