...
IF (OENTR_DETR) THEN
  ...
  IF (PARAMMF%LPZ_EXP_LOG) THEN
    ...
    DO JK=1, IKT
      ...
      ZPRES_F (JI, JK)=EXP (ZPRES_F (JI, JK))
  ...
  IF (PARAMMF%LGZ) THEN
    ...
    ZSURF =TANH (PARAMMF%XGZ*SQRT (PDX*PDY)/ZLUPSURF (JI))
...
DO JK=IKB, IKE-IKL, IKL
  ...
  IF (OENTR_DETR) THEN
    ...
    ELSEIF (GTEST .AND..NOT.GTESTLCL ) THEN
      ZT=PTH_UP (JI, JK)*(ZPRES_F (JI, JK)/CST%XP00)**(CST%XRD/CST%XCPD)
      ...
      IF (PARAMMF%LPZ_EXP_LOG) THEN
        ZPART_DRY =MAX (0., MIN (1.,(LOG (ZPRES_F (JI, JK))-LOG (ZPRE&
        & ))/(LOG (ZPRES_F (JI, JK))-LOG (ZPRES_F (JI, JK+IKL)))))
      ELSE
    ...
    ELSEIF (PARAMMF%CKIC_COMPUTE=='RS08') THEN
      ...
      IF (GTEST ) THEN
        ...
        ZDRSATDT=ZLVS*PRSAT_UP (JI, JK)/(CST%XRV*ZT**2)*(1.+(CST%XRV*PRSAT_UP (JI, JK)/CST%XRD))
  ...
  IF (OENTR_DETR) THEN
    ...
    IF (GTEST ) THEN
      ...
      ZGAMMA_WUP=1.+PARAMMF%XBRIO*ZRELAX_ALPHA_COEFF **2/((1-PARAMMF%XAADVEC&
      &)*ZLVERT)*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*ZPART_DRY 
      ZDELTA_WUP=1.-PARAMMF%XBRIO*ZRELAX_ALPHA_COEFF **2/((1-PARAMMF%XAADVEC&
      &)*ZLVERT)*(PZZ (JI, JK+IKL)-PZZ (JI, JK))*ZPART_DRY 
