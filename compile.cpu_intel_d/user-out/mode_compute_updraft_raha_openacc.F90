...
CONTAINS
SUBROUTINE COMPUTE_UPDRAFT_RAHA_OPENACC (D, CST, NEBN, PARAMMF, KSV, OENTR_DETR, ONOMIXLG&
&, KSV_LGBEG, KSV_LGEND, PZZ, PDZZ, PSFTH, PSFRV, PPABSM, PRHODREF, PUM, PVM, PTKEM&
&, PEXNM, PTHM, PRVM, PTHLM, PRTM, PSVM, PTH_UP, PTHL_UP, PRT_UP, PRV_UP, PRC_UP, PRI_UP&
&, PTHV_UP, PW_UP, PU_UP, PV_UP, PSV_UP, PFRAC_UP, PFRAC_ICE_UP, PRSAT_UP, PEMF, PDETR&
&, PENTR, PBUO_INTEG, KKLCL, KKETL, KKCTL, PDEPTH, YDSTACK)
...
DO JK=IKB, IKE-IKL, IKL
  ...
  IF (GTEST ) THEN
    ZT_UP =PTH_UP (JI, JK+IKL)*PEXNM (JI, JK+IKL)
  ...
  IF (GTEST ) THEN
    PTHV_UP (JI, JK+IKL)=PTH_UP (JI, JK+IKL)*(1.+0.608*PRV_UP (JI, JK+IKL)-PRC_UP (JI, JK+IKL))
  ENDIF
...
DO JK=IKB+IKL, IKE-IKL, IKL
  ...
  IF (JK<=IALIM ) THEN
    ZALIM_STAR_TOT =ZALIM_STAR_TOT +BR_POW(ZALIM_STAR (JI, JK), 2)*ZZDZ (JI, JK)/PRHODREF (JI, JK)
  ENDIF
