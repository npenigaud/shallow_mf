...
USE MODE_SHUMAN_MF, ONLY: MZM_MF
...
IF(PARAMMF%LRELAX_ALPHA_MF) THEN
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZRELAX_ALPHA_COEFF(JIJ, JK)=1./(1.-PFRAC_UP(JIJ, JK))
    END DO
  END DO
ELSE
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PFLXZTHLMF(JIJ, JK)  = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                 & (PTHL_UP(JIJ, JK)-PFLXZTHLMF(JIJ, JK))  ! ThetaL
    PFLXZRMF(JIJ, JK)    = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                 & (PRT_UP(JIJ, JK)-PFLXZRMF(JIJ, JK))  ! Rt
    PFLXZTHVMF(JIJ, JK)  = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                 & (PTHV_UP(JIJ, JK)-PFLXZTHVMF(JIJ, JK))  ! ThetaV

    ZFLXZTHSMF(JIJ, JK)  = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                 & (ZTHS_UP(JIJ, JK)-ZTHSM(JIJ, JK))    ! Theta S flux
  END DO
END DO
...
IF (PARAMMF%LMIXUV) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PFLXZUMF(JIJ, JK)  = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                 & (PU_UP(JIJ, JK)-PFLXZUMF(JIJ, JK))  ! U
      PFLXZVMF(JIJ, JK)  = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                 & (PV_UP(JIJ, JK)-PFLXZVMF(JIJ, JK))  ! V
    END DO
  END DO
ELSE
  PFLXZUMF(:,:) = 0.
...
IF (PARAMMF%LMIXTKE) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PFLXZTKEMF(JIJ, JK) = PEMF(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ, JK)*&
                                  & (PTKE_UP(JIJ, JK)-PFLXZTKEMF(JIJ, JK))
    END DO
  END DO
ELSE
  PFLXZTKEMF(:,:) = 0.
