...
&, PUM, PVM, PTKEM, PTHM, PRVM, PTHLM, PRTM, PSVM, PTH_UP, PTHL_UP, PRT_UP, PRV_UP, PRC_UP&
&, PRI_UP, PTHV_UP, PW_UP, PU_UP, PV_UP, PSV_UP, PFRAC_UP, PFRAC_ICE_UP, PRSAT_UP, PEMF&
&, PDETR, PENTR, PBUO_INTEG, KKLCL, KKETL, KKCTL, PDEPTH, YDSTACK)
...
USE MODE_SHUMAN_MF_OPENACC,ONLY:MZF_MF_OPENACC, MZM_MF_OPENACC, GZ_M_W_MF_OPENACC
USE MODE_COMPUTE_BL89_ML_OPENACC,ONLY:COMPUTE_BL89_ML_OPENACC

USE BR_INTRINSICS, ONLY : BR_EXP, BR_LOG, BR_POW
#include "stack.h"
USE STACK_MOD
USE ABOR1_ACC_MOD
...
IF (ZWTHVSURF>0.010) THEN
  PEMF (JI, IKB)=PARAMMF%XCMF*ZRHO_F (JI, IKB)*BR_POW(((ZG_O_THVREF&
...
DO JK=IKB, IKE-IKL, IKL
  ...
  ZBUF (IEXN)=BR_POW((PPABSM (JI, JK)/CST%XP00), CST%RDSCPD)
  ...
  ZBUF (ILVOCPEXN)=(CST%XLVTT+(CST%XCPV-CST%XCL)*ZDELT)/ZVAR2
  ZBUF (ILSOCPEXN)=(CST%XLSTT+(CST%XCPV-CST%XCI)*ZDELT)/ZVAR2
  PTH_UP (JI, JK)=PTHL_UP (JI, JK)+ZBUF (ILVOCPEXN)*ZRC_UP +ZBUF (ILSOCPEXN)*ZRI_UP 
  ...
  DO II=1, JITER
    ...
    ZBUF (IT)=PTH_UP (JI, JK)*ZBUF (IEXN)
    ...
    ZBUF (ILSOCPEXN)=(CST%XLSTT+(CST%XCPV-CST%XCI)*(ZBUF (IT)-CST%XTT))/ZVAR2
    PTH_UP (JI, JK)=PTHL_UP (JI, JK)+ZBUF (ILVOCPEXN)*ZRC_UP +ZBUF (ILSOCPEXN)*ZRI_UP 
    ZVAR1=PTH_UP (JI, JK)*ZBUF (IEXN)-ZBUF (IT)
    ZRSATW =ZRSATW +ZBUF (IDRSATODTW)*ZVAR1
    ZRSATI =ZRSATI +ZBUF (IDRSATODTI)*ZVAR1
  ...
  IF (GTEST ) THEN
    PTHV_UP (JI, JK)=PTH_UP (JI, JK)*(1.+ZRVORD*ZRV_UP )/(1.+PRT_UP (JI, JK))
  ...
  IF (GTEST ) THEN
    PEMF (JI, JK+IKL)=PEMF (JI, JK)*BR_EXP (ZMIX1 )
  ENDIF
