...
double erf(double x)
{
  if (x >=0.) return 1.0 - __erfccheb(x);
  else return __erfccheb(-x) - 1.0;
}

#pragma acc routine seq
double __erfccheb(double z){
        const double cof[28] = 
	...
	t = 2./(2.+z);
	ty = __BITREPFMA(4.,t,-2.);
	for (j=ncof-1;j>0;j--) {
		tmp = d;
		d = __BITREPFMA(ty,d,-dd) + cof[j];
		dd = tmp;
	}
        double inter = __BITREPFMA(ty,d,cof[0]);
        double z2 = z*z ;
        inter = __BITREPFMA(0.5,inter,-z2);
	return t*exp( inter - dd);
}
